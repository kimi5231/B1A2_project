...
void UMain::RecvUpdateStateMonster(S_UpdateObjectState_Packet packet)
{
	AsyncTask(ENamedThreads::GameThread, [=, this]()
	{
		...
		if (findMonster && *findMonster)
		{
			...
			if (className.Contains(TEXT("PollutionMonitor")))
			{
				if (packet.state == ObjectState::IDLE || packet.state == ObjectState::CHECK)
				{
			...
			else if (className.Contains(TEXT("Ghost")))
			{
				if (packet.state == ObjectState::IDLE || packet.state == ObjectState::ABSENT)
				{
					monster->SetActorHiddenInGame(true);
				}
				else
